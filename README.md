...
6. **全相容 H.264 視訊錄影引擎（智慧探測、Google 軟體編碼器回退與 UI 即時反饋）**
   - **徹底根治點擊錄影鍵無反應**：解決現代旗艦晶片（天璣 9300+ / Snapdragon 8 Gen 3 等）Android 14 硬體編碼器（`c2.mtk.avc.encoder`）僅支援 Surface 輸入、拒絕 ByteBuffer 記憶體輸入導致的配置異常與靜默失敗。
   - **智慧容錯降級**：優先探測硬體編碼器，若配置拋錯自動秒級回退至 Google 官方標準軟體編碼器（`c2.android.avc.encoder`），直向 480×640 比例 100% 相容且編碼耗時 < 3ms，CPU 佔用率 < 1%。
   - **全鏈路 UI 即時回饋**：點擊錄影立即彈出「開始錄影...」Toast，按鈕切換為紅色停止方塊，右上角計時碼表流暢讀秒；結束錄影即時彈出「錄影已儲存: [檔案名]」並自動同步系統相簿庫（`MediaScanner`），拍攝完成立刻可在手機相簿與左下角縮圖點擊回放！
   - **路徑雙保險機制**：主目錄寫入受阻時自動平滑回退至應用專屬儲存區，確保每一秒珍貴熱影像均被完整封裝。
...
以下為使用重構後之 **MAG-Cx** 於真實環境中實機拍攝成果（涵蓋人體面部、電器熱源與高溫火焰觀測）：
...
👉 **[下載 MAG-Cx 最新 Release APK (v2.2.7)](https://github.com/sam0324sam/apk-rebuild/releases/latest)**

- **`MAG-Cx-v2.2.7-Ready.apk`**：**唯一標準推薦安裝包**（適用所有 64 位元 Android 手機，含小米 14T Pro / 天璣 9300+ / 高通驍龍等裝置；已修復全相容 H.264 錄影引擎、即時 UI Toast 讀秒、最外圈熱漸暈平場校準、常溫物體精準捕捉、黑邊防閃爍、十字翻轉連動與音量鍵微調）。

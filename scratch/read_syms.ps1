...
for ($i = 0; $i -lt $e_shnum; $i++) {
    ...
    if ($secName -eq ".dynsym") {
        $dynsym_sec = @{
            offset = [BitConverter]::ToUInt32($bytes, $sec_offset + 0x10)
            size = [BitConverter]::ToUInt32($bytes, $sec_offset + 0x14)
            entsize = [BitConverter]::ToUInt32($bytes, $sec_offset + 0x24)
        }
    }
...
$target = "_ZN10Controller17IsOutputDataReadyEv"
...
for ($i = 0; $i -lt $count; $i++) {
    ...
    if ($symName -eq $target -or $symName -like "*GetOutputBMPData*" -or $symName -like "*PushFrame*") {
